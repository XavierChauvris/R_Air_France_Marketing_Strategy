...
library(readxl)
air_france <- read_excel("C:/Users/xavie/Downloads/Air France Case Spreadsheet Supplement.xls",
                        sheet = 2)
#Viewing Results
View(air_france)
...
#Campaigns ROA
campaign_roa <- air_france%>%
  group_by(Campaign)%>%
  select(netrevenue, TotalCost)%>%
  summarise(sum(netrevenue)/sum(TotalCost))
...
prediction_test <- predict(air_france_logit, air_france_test, type='response')
confusionMatrix(data=as.factor(as.numeric(prediction_test > 0.5)),
                reference= as.factor(as.numeric(air_france_test$binom_netrevenue)))


library(ROCR)
pred_val_logit <- prediction(prediction_test, air_france_test$binom_netrevenue )
...
publisher_cost%>%                                              #calling data frame to use from
  ...
  geom_bar(stat="identity", fill="#f68060")+                   #defining type of bar chart
  ggtitle("Publisher Average Total Cost Per Transaction ")+    #graph title
...
############################
#Checking columns name
colnames(campaign_roa)

#Changing columns name
colnames(campaign_roa)[colnames(campaign_roa) == 'sum(netrevenue)/sum(TotalCost)'] <- 'ROA'

#Checking Results
colnames(campaign_roa)

campaign_roa%>%                                              #calling data frame to use from
  mutate(Campaign = fct_reorder(Campaign, ROA))%>%           #ordering data
  ggplot( aes(x=(ROA), y=as.factor(Campaign)))+              #assigning variables to axis
  geom_bar(stat="identity", fill="#f68060")+                 #defining type of bar chart
  ggtitle("Campaign ROA")+                                   #graph title
  xlab("ROA")+                                               #x axis label
  ylab("Campaign")                                           #y axis label
